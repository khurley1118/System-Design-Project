function newsDiv(course) {
    popNews(course);
    document.getElementById('pageName').innerHTML = course;
    ...
    var len = document.getElementById('newsHolder').clientHeight;
    var len2 = len + 250;
...
function popContent(course, topic) {
    $.ajax({
        ...
        url: 'ContentPull.php',
        data: {
            givenPath: "'Content\\\\" + course + "\\\\" + topic + "'"
        },
        cache: false,
            ...
            for (var i = 0; i <= data.length - 1; i++) {
                for (var b = 0; b < 4; b++) {
                    ...
                    if (count == 1) {
                        ...
                        //File name used for checking file type
                    } else if (count == 3) {
                        output = output + data[i][b] + "<BR><BR>";
                    } else if (count == 4) {
                        var countDiff = b - 2;
...
function popNews(course) {
    $.ajax({
        ...
        success: function(data) {
            ...
            var data = JSON.parse(data);
            var output = "<br><h5 class='w3-opacity'><span class='fa fa-calendar fa-fw w3-margin-right'></span><b>News</b></h5><br><br><p>";
            for (var i = 0; i < data.length; i++) {
                output = output + data[i];
            }
            var output = output + "</p><hr>";
